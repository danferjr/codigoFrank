...
void InitServer(){
   ...
   server.serveStatic("/js/highcharts.js", SPIFFS, "/js/highcharts.js").setDefaultFile("js/highcharts.js").setCacheControl("max-age=600");
    // CSS
    ...
    // -------------------------------------------------------------------
     ...
     server.on("/grafica.html", HTTP_GET, [](AsyncWebServerRequest *request){
        AsyncResponseStream *response;
        if(false == requestPreProcess(request, response)) {
            return;
        }
        // Cargar página html Configuración del MQTT
        File file = SPIFFS.open(F("/grafica.html"), "r");
         String s = file.readString();
        request->send(200, "text/html", s);
    });
